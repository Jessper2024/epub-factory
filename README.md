...
- **首次成书门槛**：一个号攒够 **30 篇** XHTML 才出第一本书（`--min-articles N` 可改，`0` = 关掉门槛）。
  只管"首次"——**已有成品的书不受门槛约束**，照常跟着新增更新，不会停在旧版本没人发现。
  被门槛挡下的会写明「暂缓成书 · 差 N 篇」（处理报告 / 台账 / 看板三处都写），
  想立刻出书就 `./epub.sh only 号名`——**手动点名一律无视门槛**。
